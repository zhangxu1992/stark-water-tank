        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-text-secondary mt-1">Dongguan, Guangdong, China</div>
                  </div>
                </div>
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-text-secondary mt-1">info@starkwatertank.com</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-border shadow-sm p-8">
              ...
              {status === 'success' && (
                <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg text-sm text-green-700">{t('formSuccess')}</div>
              )}
              {status === 'error' && (
                <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg text-sm text-red-700">{t('formError')}</div>
              )}
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required value={form.name} onChange={e => update('name', e.target.value)} className={fCls}/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1.5">{t('formEmail')} *</label>
                    <input required type="email" value={form.email} onChange={e => update('email', e.target.value)} className={fCls}/>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1.5">{t('formPhone')}</label>
                    <input value={form.phone} onChange={e => update('phone', e.target.value)} className={fCls}/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1.5">{t('formCountry')}</label>
                    <input value={form.country} onChange={e => update('country', e.target.value)} className={fCls}/>
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1.5">{t('formCompany')}</label>
                  <input value={form.company} onChange={e => update('company', e.target.value)} className={fCls}/>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1.5">{t('formMessage')} *</label>
                  <textarea required value={form.message} onChange={e => update('message', e.target.value)} rows={5} className={fCls + ' resize-y'}/>
                </div>
